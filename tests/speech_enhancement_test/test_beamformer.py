...
class TestBeamformerMethods(unittest.TestCase):
    ...
    def generate_source_file_with_matlab(mlab = Mlab()):
        ...
        mlab.run_code("noise = database.noise(tSignal, samplingRate, D, seed, 'noiseType', noiseType, 'sensorPositions', sensorPositions);")

        mlab.run_code("[~, speakers, noise] = bss.generate.convolutiveMixture(speakers, rir, noise, SNR, 'sourceScaled');")
    ...
    def test_compare_lcmv_beamformer(self):
        ...
        mlab.run_code("[W(:, :, 1), failing] = bss.beamformer.lcmv('observationMatrix', Phi_NN, 'lookDirection', W_pca, 'responseVector', [1 0]);")
        mlab.run_code("W(:, :, 2) = bss.beamformer.lcmv('observationMatrix', Phi_NN, 'lookDirection', W_pca, 'responseVector', [0 1]);")
        W_matlab = mlab.get_variable('W')
