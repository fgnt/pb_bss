...
from pb_bss.distribution.complex_bingham_utils import grad_log_norm_symbolic
...
class ComplexBingham(_ProbabilisticModel):
    ...
    def norm(self, remove_duplicate_eigenvalues=True):
        """
        ...
        if remove_duplicate_eigenvalues:
            _, covariance_eigenvalues = self._remove_duplicate_eigenvalues(covariance_eigenvalues)
    ...
    def _remove_duplicate_eigenvalues(cls, covariance_eigenvalues, eps=1e-8):
        ...
        covariance_eigenvalues[..., :-1] = (
                covariance_eigenvalues[..., -1][..., None]
                - np.cumsum(diff[..., ::-1], axis=-1)[..., ::-1]
        )
...
class ComplexBinghamTrainer:
    ...
    def _fit(self, y, saliency) -> ComplexBingham:
        ...
        for index in np.ndindex(scatter_eigenvalues.shape[:-1]):
            eigenvalues[index] = self.find_eigenvalues_v2(scatter_eigenvalues[index])
